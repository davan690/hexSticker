...
```{r eval=FALSE}
library(hexSticker)
s <- sticker(package="hexSticker", p_size=20, s_x=.8, s_y=.6, s_width=1.4, s_height=1.2,
          filename="inst/figures/cat.png")
```
...
Ok so this is the dumbass version of steps...

1. file location of photo..

```{r fig.keep="none", message=FALSE, eval=FALSE}
imgurl <- "C://Code/hexSticker/inst/figures/cat.png"
?sticker
```
...
s <- sticker(imgurl, package="SSN", p_size=20, s_x=1, s_y=.75, s_width=.6, subplot = "inst/figures/cat.png")
s
```
